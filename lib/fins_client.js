...
_udpWrite = function(socket, buffer, port, host, callback) {
  socket.send(buffer, 0, buffer.length, port, host, callback);
};

_tcpWrite = function(socket, buffer, callback) {
...
FinsClient.init = function (port,host,options) {
    ...
    this.protocol = (options && options.protocol) || defaultOptions.protocol;
    switch(this.protocol){
      ...
      case 'tcp':
        // Create net socket here with correct listeners
        this.socket = net.createConnection(this.port, this.host, listening);
      ...
      default:
        throw new Error(
          'Incorrect option',
          options.protocol,
          'for protocol, only "tcp" or "udp" allowed'
        );
    }
    ...
    function receive (buf,rinfo) {
      ...
      var msg = _processReply(buf,remoteInfo);
      self.emit('reply',msg);
    }
...
FinsClient.prototype.read = function(address,regsToRead,callback) {
    ...
    if (this.protocol === 'tcp') {
      _tcpWrite(this.socket, buffer, callback);
    } else if(this.protocol === 'udp') {
      _udpWrite(this.socket, buffer, self.port, self.host, callback);
    }
};
